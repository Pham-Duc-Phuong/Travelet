import { Swiper, SwiperSlide } from 'swiper/react'; import { Autoplay } from 'swiper/modules'; import 'swiper/css'; import 'swiper/css/pagination'; import { useEffect, useState } from 'react'; import { useAppDispatch, useAppSelector } from 'store'; import { LocationByPageThunk, LocationThunk } from 'store/Location'; import '../layout/style.css'; import { generatePath, useNavigate } from 'react-router-dom'
    ...
    <div>
      <div className='relative'>
        ...
        <div className='sm:absolute sm:-bottom-[38px] sm:left-1/2 sm:-translate-x-1/2 max-w-screen-lg m-auto sm:bg-white sm:border sm:rounded-[40px] flex flex-col sm:flex-row justify-between items-center w-full my-2 sm:my-0'>
          <form className='grid grid-col-1 sm:grid-cols-search w-full mx-3 sm:mx-5 items-center'>
            ...
            <button type='submit' className='mt-1 w-auto mx-4 py-2 sm:mt-0 sm:mx-3 sm:w-fit sm:h-fit sm:px-3 rounded-md sm:rounded-[50%] border bg-gradient-to-br from-cyan-300 to-blue-600'><i className="fa-solid fa-magnifying-glass text-white"></i></button>
          </form>
        </div>
      </div>
      {/* Explore Nearby Destinations */}
      <div className='max-w-screen-xl m-auto sm:pt-8 px-4 sm:px-[30px]'>
          ...
          <div className='grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-5'>
            {
              locationByPage?.map(a => (
                <div key={a.id} className='bg-white rounded-lg hover:bg-cyan-100 transition-all duration-500 cursor-pointer' onClick={() => {
                  const path = generatePath(PATH.location, { locationName: a.tinhThanh, locationID: a.id })
                  navigate(path)
                }}>
          ...
          >
            {
              TouristAttraction?.map((a, index) => (
                <SwiperSlide key={index}><div className='relative px-3 sm:hover:-translate-y-4 sm:transition-all sm:duration-700'><div className='div-carousel'><p className='title-carousel'>{a.viTri}</p><p className='p-carousel'>{a.tinhThanh}</p></div><img className='img-carousel' src={`/images/carousel-${index + 1}.jpg`} alt="" /></div></SwiperSlide>
              ))
            }
          </Swiper>
        </div>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-8'>
            {
              TouristHobby?.map((a, index) => (
                <div key={index} className='relative hover:-translate-y-4 transition-all duration-700'>
